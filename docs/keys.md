...
* [`dvc keys rotate`](#dvc-keys-rotate)
...
Rotate SDK keys for an environment

```
USAGE
  $ dvc keys rotate [--config-path <value>] [--auth-path <value>] [--repo-config-path <value>] [--client-id
    <value>] [--client-secret <value>] [--project <value>] [--no-api] [--headless] [--env <value>] [--type
    client|mobile|server]

FLAGS
  --env=<value>                  Environment to rotate keys for
  --type=(client|mobile|server)  The type of SDK key to rotate
